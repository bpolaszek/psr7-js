...
function extract_path(key: string) {
  ...
  return [decodeURIComponent(path), ...segments];
}
...
export function parse_query_string(
  ...
  for (const segment of segments) {
    ...
    let valuePart =
      operatorPosition === segment.length
        ? null
        : segment.substring(operatorPosition + 1);
...
function render_query_string_pairs(
  qs: any,
  pairs: Array<string> = [],
): Array<string> {
  for (const key of Object.keys(qs)) {
    ...
    if (null === value) {
      ...
      pairs.push(
        ...value.map(
          (value) =>
            `${encodeURIComponent(key)}%5B%5D=${encodeURIComponent(value)}`,
        ),
      );
    } else if ("object" === typeof value) {
      for (const [segments, _value] of browse_until_I_get_a_scalar(value)) {
        const keyPart = `${key}${segments
          .map((key: string) => `[${key}]`)
          .join("")}`;
        if (null === _value) {
          ...
          pairs.push(
            `${encodeURIComponent(keyPart)}=${encodeURIComponent(_value)}`,
          );
        }
      }
    } else {
